...
plt.tight_layout(); plt.savefig(f'{D}/n1-jalur.png', dpi=200); plt.close()
...
plt.tight_layout(); plt.savefig(f'{D}/n2-tema.png', dpi=200); plt.close()
...
plt.tight_layout(); plt.savefig(f'{D}/n3-penanganan.png', dpi=200); plt.close()
...
plt.tight_layout(); plt.savefig(f'{D}/n4-asal.png', dpi=200); plt.close()
...
plt.tight_layout(); plt.savefig(f'{D}/n5-bulan.png', dpi=200); plt.close()
...
WordCloud(width=1600, height=900, background_color='white', prefer_horizontal=.92,
         ...
         ).generate_from_frequencies(dict(freq.most_common(70))).to_file(f'{D}/n6-awankata.png')
...
json.dump({'stat': stat, 'daftar': daftar}, open(f'{D}/data.json', 'w'), indent=1, ensure_ascii=False)
