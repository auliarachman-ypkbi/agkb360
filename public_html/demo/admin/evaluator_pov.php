...
<style>
.pov-hero{background:radial-gradient(520px 320px at 12% 0%,rgba(34,1,178,.5) 0%,transparent 68%),radial-gradient(420px 300px at 100% 110%,rgba(255,145,1,.18) 0%,transparent 62%),linear-gradient(150deg,#02001f 0%,#040136 55%,#030870 100%);color:#fff;border-radius:16px;padding:24px 28px;margin-bottom:20px;display:flex;align-items:center;gap:18px}
...
.back-link{font-size:12px;color:#6b6a83;text-decoration:none;display:inline-flex;align-items:center;gap:5px;margin-bottom:14px}
.back-link:hover{color:#040136}

.stat-row{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-bottom:18px}
.stat-box{background:#fff;border:0.5px solid #e3e5ea;border-radius:10px;padding:14px;text-align:center}
.stat-box-val{font-size:24px;font-weight:600}
.stat-box-lbl{font-size:11px;color:#6f6e85;margin-top:2px}

.status-tabs{display:flex;gap:6px;margin-bottom:14px}
.status-tab{padding:6px 14px;border-radius:20px;font-size:12px;font-weight:500;border:0.5px solid #e3e5ea;background:#fff;color:#6b6a83;text-decoration:none}
.status-tab.active{background:#040136;color:#fff;border-color:#040136}

.pov-table{width:100%;border-collapse:collapse;font-size:13px;background:#fff;border:0.5px solid #e3e5ea;border-radius:12px;overflow:hidden}
.pov-table thead th{background:#fafafb;color:#2f2d4d;font-weight:600;font-size:11px;text-transform:uppercase;letter-spacing:.4px;padding:11px 16px;text-align:left;border-bottom:1px solid #e3e5ea}
.pov-table tbody td{padding:12px 16px;vertical-align:middle;color:#040136;border-bottom:0.5px solid #f3f4f6}
.pov-table tbody tr:hover{background:#fafafb}
...
.pkg-badge{display:inline-block;background:#040136;color:#fff;font-size:11px;font-weight:600;padding:3px 10px;border-radius:6px}
.resp-sub{font-size:11px;color:#6f6e85;margin-top:4px;display:block}
.mini-bar-wrap{width:70px}
.mini-bar-track{height:5px;border-radius:3px;background:#e3e5ea;overflow:hidden;margin-bottom:3px}
.mini-bar-fill{height:100%;border-radius:3px;background:#d97701}
.mini-bar-lbl{font-size:10px;color:#6f6e85}
.action-icon-btn{width:30px;height:30px;border-radius:7px;display:inline-flex;align-items:center;justify-content:center;border:1px solid #e3e5ea;background:#fff;color:#6b6a83;text-decoration:none}
...
<div class="stat-row">
  ...
  <div class="stat-box"><div class="stat-box-val" style="color:#027a48"><?= $sumMap['completed'] ?? 0 ?></div><div class="stat-box-lbl">Selesai</div></div>
  <div class="stat-box"><div class="stat-box-val" style="color:#b42318"><?= ($sumMap['pending'] ?? 0) + ($sumMap['in_progress'] ?? 0) ?></div><div class="stat-box-lbl">Belum Selesai</div></div>
</div>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        <?php else: ?>
        <span style="color:#cdd0d8;font-size:12px">—</span>
        <?php endif; ?>
      </td>
      <td style="font-size:12px;color:#2f2d4d;white-space:nowrap"><?= $a['due_date'] ? date('d M Y', strtotime($a['due_date'])) : '—' ?></td>
